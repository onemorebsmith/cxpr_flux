...
namespace cxpr_flux
{
	...
	template <size_t slab_size = 1024 * 8, bool throwOOM = true>
	...
	public:
		...
		constexpr arena_allocator() noexcept : control{}
		{
		...
		void purge()
		{
			...
			{
				deallactor_entry_node* node = reinterpret_cast<deallactor_entry_node*>(((char*)(&control.memstart)) + control.currentHeadOffset);
				while (true)
				{
					node->destruct();
	...
	private:
		...
		{
			std::atomic<__int64> currentHeadOffset = 0; // todo: atomic this
